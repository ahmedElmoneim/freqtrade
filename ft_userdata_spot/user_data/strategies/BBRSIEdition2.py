...
class BBRSIEDITION2(IStrategy):
    ...
    trailing_stop_positive_offset = 0.269  # Disabled / not configured
    ...
    buy_rsi_enable = CategoricalParameter([True, False],default=True,space="buy", optimize=True)
    buy_rsi_value =  IntParameter(low=5, high=50, default=25, space="buy", optimize=True, load=True)
    ...
    buy_BB = DecimalParameter(1, 4, decimals=1, default=3.5, space="buy")

    sell_rsi_enable = CategoricalParameter([True, False],default=True,space="sell", optimize=True)
    sell_rsi_value = IntParameter(low=30, high=99, default=40, space="sell", optimize=True, load=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        bollingerBuy = qtpylib.bollinger_bands(qtpylib.typical_price(dataframe), window=20, stds=self.buy_BB.value)
        dataframe['bb_lowerbandBuy'] = bollingerBuy['lower']
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if(self.buy_rsi_enable):
            ...
            # condation = (dataframe["trough"]==True)& (dataframe['rsi'] > self.buy_rsi_value.value) & (dataframe["close"] < dataframe["bb_lowerbandBuy"])
            condation =  (dataframe['rsi'] > self.buy_rsi_value.value) & (dataframe["close"] < dataframe["bb_lowerbandBuy"])

        else:
            # print(f"currunt:{dataframe["trough"]},shift:{dataframe["trough"].shift(1)}")
            # condation = (dataframe["trough"]==True)& (dataframe["close"] < dataframe["bb_lowerbandBuy"])
            condation =  (dataframe["close"] < dataframe["bb_lowerbandBuy"])
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if(self.sell_rsi_enable):
            ...
            condation=(dataframe['rsi'] > self.sell_rsi_value.value) & (dataframe["close"] > dataframe[self.sell_trigger.value] )

        else:
            # condation=(dataframe["peak"]==True)&(dataframe["close"] > dataframe[self.sell_trigger.value])
            condation=(dataframe["close"] > dataframe[self.sell_trigger.value])
